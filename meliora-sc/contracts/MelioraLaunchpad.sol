...
contract MelioraLaunchpad {
    ...
    IUniswapV2Router02 private constant uniswapRouter =IUniswapV2Router02(address(0xD99D1c33F9fC3444f8101754aBC46c52416550D1));

      address payable internal melipadFactoryAddress; // address that creates the presale contracts
    address payable public melipadDevAddress; // address where dev fees will be transferred to
    address public melipadLiqLockAddress; // address where LP tokens will be locked
    ...
    uint256 private melipadDevFeePercentage; // dev fee to support the development of melipad Investments
    uint256 public melipadId; // used for fetching presale without referencing its address
    ...
    bool public melipadDevFeesExempted = false; // if true, presale will be exempted from dev fees
    ...
    constructor(address _melipadFactoryAddress, address _melipadDevAddress) public {
        require(_melipadFactoryAddress != address(0));
        require(_melipadDevAddress != address(0));

        melipadFactoryAddress = payable(_melipadFactoryAddress);
        melipadDevAddress = payable(_melipadDevAddress);
    }

    modifier onlymelipadDev() {
        require(melipadFactoryAddress == msg.sender || melipadDevAddress == msg.sender);
        _;
    }

    modifier onlymelipadFactory() {
        require(melipadFactoryAddress == msg.sender);
        _;
    }

    modifier onlyPresaleCreatorOrmelipadFactory() {
        require(
            presaleCreatorAddress == msg.sender || melipadFactoryAddress == msg.sender,
    ...
    ) external onlyPresaleCreatorOrmelipadFactory {
    ...
    function setmelipadInfo(
        address _melipadLiqLockAddress,
        uint256 _melipadDevFeePercentage,
        uint256 _melipadId
    ) external onlymelipadDev {
        melipadLiqLockAddress = _melipadLiqLockAddress;
        melipadDevFeePercentage = _melipadDevFeePercentage;
        melipadId = _melipadId;
    }

    function setmelipadDevFeesExempted(bool _melipadDevFeesExempted)
    external
    onlymelipadDev
    {
        melipadDevFeesExempted = _melipadDevFeesExempted;
    }

    function setOnlyWhitelistedAddressesAllowed(bool _onlyWhitelistedAddressesAllowed)
    external
    onlyPresaleCreatorOrmelipadFactory
    {
    ...
    function addLiquidityAndLockLPTokens() external presaleIsNotCancelled {
        ...
        uint256 melipadDevFeeInWei;
        if (!melipadDevFeesExempted) {
            uint256 pctDevFee = finalTotalCollectedWei.mul(melipadDevFeePercentage).div(100);
            melipadDevFeeInWei = pctDevFee;
        }
        if (melipadDevFeeInWei > 0) {
            finalTotalCollectedWei = finalTotalCollectedWei.sub(melipadDevFeeInWei);
            melipadDevAddress.transfer(melipadDevFeeInWei);
        }
    ...
    function cancelAndTransferTokensToPresaleCreator() external {
        if (!uniLiquidityAdded && presaleCreatorAddress != msg.sender && melipadDevAddress != msg.sender) {
            revert();
        }
        if (uniLiquidityAdded && melipadDevAddress != msg.sender) {
            revert();
        }
